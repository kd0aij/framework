...
/*
  erase a page
 */
bool flash_erasepage(void* page_addr)
{
    if ((uint32_t)page_addr >= STM32_FLASH_NPAGES) {
        return false;
    }
    ...
    uint8_t snb = ((((uint32_t)page_addr % 12) << 3) | (((uint32_t)page_addr / 12) << 7));
    ...
    stm32_flash_lock();
    return stm32_flash_ispageerased((uint32_t)page_addr);
}
